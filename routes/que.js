						...
						Que.find({ quizId: quizId }, "desc opt points author type negPoint")
							.then(ques => {
								...
								if (quiz.random.isRandom == true) {
									...
									let singleChoiceCount =
										quiz.random.singleChoice.count < ques.length
											? quiz.random.singleChoice.count
											: ques.length;
									let multipleChoiceCount =
										quiz.random.multipleChoice.count < ques.length
											? quiz.random.multipleChoice.count
											: ques.length;
									singleChoiceQues = singleChoiceQues
										.sort(() => 0.5 - Math.random())
										.slice(0, singleChoiceCount);
									multipleChoiceQues = multipleChoiceQues
										.sort(() => 0.5 - Math.random())
										.slice(0, multipleChoiceCount);
...
router.get("/viewSol/:id", Auth.authenticateAll, (req, res, next) => {
	Que.findById(req.params.id, "sol")
		.exec()
		.then(result => {
			res.status(200).json({
				status: 1,
							...
							Que.find({ quizId: req.body.quizId }, (err, ques) => {
								...
								for (var i = 0; i < userSub.length; i++) {
									...
									if (userSub[i].ans.length > 0) {
										...
										if (que.type == 3) {
											if (
												userSub[i].ans[0].toLowerCase().trim() === que.sol[0].toLowerCase().trim()
											) {
